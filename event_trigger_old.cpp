...
void showMenu(int pin, int64_t duracao){     
    ...
    std::cout << " 1 - Ler os biases da câmera" << std::endl;
    std::cout << " 2 - Ativar o pino "<<  pin << " da Jetson (3,3V)"<< std::endl;        
    std::cout << " 3 - Desativar o pino "<<  pin << " da Jetson (0,0V)"<< std::endl;
    std::cout << " 4 - Iniciar gravacao de eventos com trigger (.raw)" << std::endl;
...
void readCameraBiases(Metavision::Camera &cam) {
    try {
        ...
        if (biases) {
            ...
            // Lista de biases comuns no IMX636 para você verificar manualmente
