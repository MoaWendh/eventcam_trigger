...
struct LineJetson{
    ...
    const int lineA= 43;  // GPIO13 PH.00  - Pino IO no Header= 33
    const int lineB= 85;  // GPIO12 PN.01  - Pino IO no Header= 15
    const int lineC= 105; // GPIO01 PQ.05  - Pino IO no Header= 29
    const int lineD= 41;  // GPIO07 PG.06  - Pino IO no Header= 32
    const int lineE= 122; // GPIO01 PY.00  - Pino IO no Header= 13         
    const int lineF= 144; // GPIO09 PAC.06 - Pino IO no Header= 7      
    const int lineG= 106; // GPIO11 PQ.06  - Pino IO no Header= 31
    const int lineH= 126; // GPIO01 PY.04 - Pino IO no Header= 16
};

struct PinoJetson{
    // Nº do pino físico no barramento (conector j12) de GPIO da Jetson:
...
int main(int argc, char *argv[]) {
    ...
    LineJetson line_jetson;

    PinoJetson pino_jetson;
